...
namespace astareuc
{
	...
	template <class T>
	void Solution::Astar() {
		int start_code = Step::convert_to_code(start, size.second);
		open_list.insert(Step(start_code, 1));
		process.push_back(Step(start_code, 1));
		open[start_code] = true;
		parent[start_code] = Step(-1);
