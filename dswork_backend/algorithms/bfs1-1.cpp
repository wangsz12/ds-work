...
{	
	...
	{		
		...
		public:
			...
			bool check_que(pair<int,int> point,queue<Node> que,int a);
	};
		...
		{			
				...
				while(!que.empty())
			{
				...
				{					 
					...
					!check_que(make_pair(tmp.getx(),tmp.gety()),que,a[count].layer))
					{
						tmp.pre=&a[count];
	...
	bool Solution::check_que(pair<int,int> point,queue<Node> que,int a)
	{
		while(!que.empty())
		{
			Node tp=que.front();
			if(point.first==tp.getx()&&point.second==tp.gety()&&a==tp.layer)
			return true;
			else que.pop();
		}
		return false;
